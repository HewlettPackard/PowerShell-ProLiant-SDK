...
$target = Read-Host "Enter target to get the log [Accepted values (OA, Enclosure)]`t"

$rt = $connection | Get-HPEOASysLog -Target $target
$logCount = Read-Host "Enter number of log you want to see`t"
Write-Host "==== Sys log for $target ====" -ForegroundColor Yellow
foreach ($oa in $rt) 
{
    $oaip=$oa.IP
    $systemLog = $oa.Log | Select-Object -Last $logCount 
    if($systemLog.Count -eq 0)
    {
        Write-Host "No log found for : $target" -ForegroundColor Red
    }
    else
    {
       $systemLog
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDYrUXEePsF5wid
# JefDudEuJ/MSi/g3c/mhsV/so9+rAaCCHrIwggPuMIIDV6ADAgECAhB+k+v7fMZO
...
# BAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIAYqWb4br+H4
# Q30KLWGX3Rr/mKXv/otxJqDjcYkng++pMA0GCSqGSIb3DQEBAQUABIIBABjwBbfh
# orIuQvpAcNwEkDQ9hYxy6SYN2YrHM/ChizRWIMONau6yU4BiT85qlJj/WSLNFyNl
# zlKtymP49cCbWqB4f+ouJQZd5FSnA9trAgNUwrogpfqDXN5PwIddcU19Iuqjm0r2
# O7Lf5gp2uP8ourQYClNCYjY1bIJj/z576fowxmJzmECa9kiahAuREH50rDD8FcOD
# d6VsPL2sIzWt/pRRQB/PgrYJ9xOa4o7xg5U8Cyl+1GsxY5l19OGrrquqCxTSTCju
# XkaZ2esBz4DI0d7HF4HmLcfdBZRwI1nL4sDk8ud+XBwSAMr3aCHNwsz0ErPJv0wf
# ncz1yqjR2kNKHsWhggILMIICBwYJKoZIhvcNAQkGMYIB+DCCAfQCAQEwcjBeMQsw
...
# DQEHATAcBgkqhkiG9w0BCQUxDxcNMTgwODAyMDUxODQwWjAjBgkqhkiG9w0BCQQx
# FgQUYsQ/6wFp2Nb4syYqNznwkuP9osswDQYJKoZIhvcNAQEBBQAEggEAHv6BRyQJ
# s+oDluGIt4OEu7vBcZzq80znzQyf9dOE0L2BM8ywy1OaYfE3WEEoSMNTDmEIZMQB
# 4oS+KKJIuIutg6/m517UNRUT+l4cPN4eHYAm6+aaJIuD9l02q72miRl64vFpZ2hU
# IeHFZhHMpaurvXIp+i8WEWPFcFIxa4EQrR4GcBzzwYwioCYnr8pIH9RtFuC7V3PC
# t2iLidVKRohfwr42jfDAm3frDJaFILk4NvVjtbCvaSRIxB8ktsoYKHULkyZ1Fb7Q
# My4PUTO6oBUUkx++ayAF4gllDh0orQwRL5qB1snUmDO/vAlu+k9Y+nZhH0KRl3xT
# WqjQBmAulDLlWg==
