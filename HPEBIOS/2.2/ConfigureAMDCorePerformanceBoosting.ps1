#################################################################
#Configuring AMD Core Performance Boosting on Gen10 servers with AMD processors
############################################################----#

<#
.Synopsis
    This script allows user to change core performance boosting on HPE Proliant Gen10 servers with AMD processors.

.DESCRIPTION
    This script allows user to change the AMD core peformance boosting.
    AMDCorePerformanceBoost :- Use this option to set AMD Core Performance Boosting configuration on HPE Proliant Gen10 servers with AMD processors.
    configurations: Enabled and Disabled.
...
.EXAMPLE
    ...
    -AMDCorePerformanceBoost:- Accepted values are Enabled or Disabled. For multiple servers values should be separated by comma(,)

.EXAMPLE
    ConfigureAMDCorePerformanceBoosting.ps1 -Address "10.20.30.40" -Credential $userCredential -AMDCorePerformanceBoost "Enabled"

    This mode of script have input parameter for Address Credential and AMDCorePerformanceBoost.
    ...
    -AMDCorePerformanceBoost:- Use this Parameter to specify AMD Core Performance Boosting configuration. Accepted values are Enabled or Disabled
...
Param(
    ...
    # AMDCorePerformanceBoost (Enabled or Disabled)
    [String[]]$AMDCorePerformanceBoost  
     )
...
Write-Host "This script allows user to change AMD Core Performance Boosting."
Write-Host ""
...
foreach($IPAddress in $ListOfAddress)
{
    ...
    if($connection -ne $null)
     {  
        ...
        $connection
        if($connection.ProductName.Contains("Gen10") -and $connection.ProcessorInfo.Contains("AMD"))
        {
            $ListOfConnection += $connection
        }
        else
        {
            Write-Host "AMD Core Performance Boosting is not supported on Server $($connection.IP)"
...
# Get current AMD Core Performance Boosting

Write-Host ""
Write-Host "Current AMD Core Performance Boosting configuration" -ForegroundColor Green
Write-Host ""
$counter = 1
foreach($serverConnection in $ListOfConnection)
{
    $result = Get-HPEBIOSProcessorOption -Connection $serverConnection
...
$parameterMetaData = $(Get-Command -Name Set-HPEBIOSProcessorOption).Parameters
$AMDCorePerformanceBoostValidValues = $($parameterMetaData["AMDCorePerformanceBoost"].Attributes | where {$_.TypeId.Name -eq "ValidateSetAttribute"}).ValidValues
...
Write-HOst ""
if($AMDCorePerformanceBoost.Count -eq 0)
{
    $tempAMDCorePerformanceBoost = Read-Host "Enter AMD Core Performance Boosting [Accepted values : ($($AMDCorePerformanceBoostValidValues -join ","))]."
    $AMDCorePerformanceBoost = $tempAMDCorePerformanceBoost.Trim().Split(',')
    if($AMDCorePerformanceBoost.Count -eq 0)
    {
        Write-Host "AMDCorePerformanceBoost is not provided`n Exit......."
        exit
    }
}
...
{
	if($($AMDCorePerformanceBoostValidValues | where {$_ -eq $AMDCorePerformanceBoost[$i]}) -eq $null)
	{
		Write-Host "Invalid AMD core performance boosting" -ForegroundColor Red
		Write-Host "Exit...."
		exit
	}
}

Write-Host "Changing AMD core performance boosting....." -ForegroundColor Green

$failureCount = 0
if($ListOfConnection.Count -ne 0)
{
	$setResult =  Set-HPEBIOSProcessorOption -Connection $ListOfConnection -AMDCorePerformanceBoost $AMDCorePerformanceBoost
    foreach($result in $setResult)
    {
	    if($result.Status -eq "Error")
	    {
		    Write-Host ""
		    Write-Host "AMD core performance boosting Cannot be changed"
...
{
	Write-Host ""
	Write-host "AMD Core Performance Boosting changed successfully" -ForegroundColor Green
	Write-Host ""
	$counter = 1
	foreach($serverConnection in $ListOfConnection)
	{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCOX/vNQPPuyKmi
# enWq7AEMdmCZHu+un3YJ8frpsZO+C6CCHuQwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# AQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCAkYrAc3lnG
# 1NuLdTeUnQopZtsV9eoVojANkm13R+xpNDANBgkqhkiG9w0BAQEFAASCAQCdUg4U
# 0/PHRPe1EZAYoVO3acjS2hOrHP4RCO2pw0PAzdYrpdmCBbVE+HAf5H4YncN5A3zQ
# inV8s0xjHOzhKM8vF+cj/XzAthRQobyQUELbGKzoELRf8XOcxrUfNQw/Ibff9IbV
# 1axglnuIV+7hHP1FXL0nSwk7Ng4ZWJ7EqwRyktWjN3r9OSCs2yIpfrBV6S15V23k
# WlzcZe/HQP/WkCBDxUT5QGlNQHuam+7t/f7prjcEEP8VuKSksuPRBMZL0SSAwM18
# n8WK4G6AvH960yshkaiHoqKPzWsv5ynxLaO17Fxq1Wb3x6l7NYqh+CJzGBJBBMFu
# fsdLVww7DukKaWCPoYICojCCAp4GCSqGSIb3DQEJBjGCAo8wggKLAgEBMGgwUjEL
# MAkGA1UEBhMCQkUxGTAXBgNVBAoTEEdsb2JhbFNpZ24gbnYtc2ExKDAmBgNVBAMT
...
# KoZIhvcNAQkFMQ8XDTE5MDQyMzA2MDU0NlowIwYJKoZIhvcNAQkEMRYEFNm6KuQo
# H5REXSNTqI23ciw94SsNMIGdBgsqhkiG9w0BCRACDDGBjTCBijCBhzCBhAQUY7gv
...
# AQAbtoddxaY2fbCNZCPsbRXJPDLQRHg3aNi+ONzn/eB8TCYabvcPoO62R++/jBwb
# M5y1cehToabGB4kDvOdiNhKgCCQp9iib1QCHTql153U5g7O9Hh6VmPNwY1ZG75W9
# 1yJvFEY+vlMO3kfKXW+V6yAwk5aaUL4W1doBE/fZNnlbfxXoXXP/dOzH4rIz+OMt
# VR0VjA/DJgpiaHVRugQ2MAwhLF+E9Ekrnzt1bQRE88kEISPkGaOCCvpRlOQ6NL7M
# ZrqBgBvnJiWRCmQPJ/qCtXjM2C8NUP4kdpxuIqvO29+V83uQbWhrnHW78tSVwyGu
# COF++RTDag31ytxCu9XRhyLX
