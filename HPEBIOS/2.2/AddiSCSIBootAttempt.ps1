########################################################
#Adding iSCSI boot attempt
###################################################----#

<#
.Synopsis
    This script allows user to add an iSCSI boot attempt on HPE Proliant Gen9 and Gen10 servers.

.DESCRIPTION
    This script allows user to add an iSCSI boot attempt.
    iSCSIBootAttemptIndex :- Use this option to specify the index to add an iSCSI boot attempt into iSCSI boot source array on HPE ProLiant Gen9 and Gen10 servers.
    iSCSINICSource :- Use this option to specify the iSCSI NIC boot source on HPE ProLiant Gen9 and Gen10 servers.

.EXAMPLE
    AddiSCSIBootAttempt.ps1
    ...
    -iSCSIBootAttemptIndex   :- Specifiy the index to add an iSCSI boot attempt into iSCSI boot source array. Accepted values are 1 to 4.
        
    -iSCSINICSource   :- Specifiy the the iSCSI NIC boot source. Accepted values are NICBoot1, NICBoot2, NICBoot3, NICBoot4, NICBoot5, NICBoot6, NICBoot7, NICBoot8, NICBoot9, NICBoot10, NICBoot11, NICBoot12, NICBoot13, NICBoot14, NICBoot15 and NICBoot16.

.EXAMPLE
    AddiSCSIBootAttempt.ps1 -Address "10.20.30.40" -Credential $userCredential -iSCSIBootAttemptIndex 1 -iSCSINICSource NICBoot1
...
Param(
    ...
    # iSCSIBootAttemptIndex parameter
    [Int32[]]$iSCSIBootAttemptIndex, 
    # iSCSINICSource parameter
    [String[]]$iSCSINICSource
     )
...
Write-Host "This script allows user to add an iSCSI boot attempt into iSCSI boot source array."
Write-Host ""
...
foreach($IPAddress in $ListOfAddress)
{
    ...
    if($connection -ne $null)
     {  
        ...
        {
            Write-Host "Add an iSCSI Boot Attempt is not supported on Server $($connection.IP)"
...
{
     $tempiSCSIBootAttemptIndex = Read-Host "Enter iSCSI Boot Attempt Index [Accepted values : (1 to 4)]"
     $iSCSIBootAttemptIndex = $tempiSCSIBootAttemptIndex.Trim().Split(',')
     if($iSCSIBootAttemptIndex.Count -eq 0){
         Write-Host "iSCSIBootAttemptIndex value is not provided`nExit....."
            Exit
     }
}

# Get the valid value list fro each parameter
$iSCSINICSourceParameterMetaData = $(Get-Command -Name Add-HPEBIOSiSCSIBootAttempt).Parameters
$iSCSINICSourceValidValue = $($iSCSINICSourceParameterMetaData["iSCSINICSource"].Attributes | where {$_.TypeId.Name -eq "ValidateSetAttribute"}).ValidValues

if($iSCSINICSource.Count -eq 0)
{
    $tempiSCSINICSource = Read-Host "Enter iSCSI NIC Source [Accepted values : ($($iSCSINICSourceValidValue -join ","))]"
    $iSCSINICSource = $tempiSCSINICSource.Trim().Split(',')
    if($iSCSINICSource.Count -eq 0)
    {
        Write-Host "iSCSINICSource is not provided`n Exit......."
        exit
    }
}

Write-Host "Adding an iSCSI boot attempt into iSCSI boot source array....." -ForegroundColor Green

$failureCount = 0
if($ListOfConnection.Count -ne 0)
{
	$setResult = Add-HPEBIOSiSCSIBootAttempt -Connection $ListOfConnection -iSCSIBootAttemptIndex $iSCSIBootAttemptIndex -iSCSINICSource $iSCSINICSource
    foreach($result in $setResult)
    {
	    if($result.Status -eq "Error")
	    {
		    Write-Host ""
		    Write-Host "iSCSI Boot Attempt Cannot be added"
...
{
	Write-Host ""
	Write-host "Added an iSCSI boot attempt successfully into iSCSI boot source array" -ForegroundColor Green
	Write-Host ""
	$counter = 1
	foreach($serverConnection in $ListOfConnection)
	{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD5oldOwaZq3G/4
# uMNh/nXbMKP2PxYWhqdeQTD9jAF1NKCCHuQwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# AQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCCqvJ0fASDE
# QZPJksM5CFCF0jD+WBDGrWW0SksZbiH2XzANBgkqhkiG9w0BAQEFAASCAQDXPiI/
# BL+pqPRZe+IcEw+rKx38PN1dW5tnepYiiXPF88Xyx4FUJbcPt3Inr4NyI9CgC7ya
# iBLT3bqmE0njkZFyFyrF9iHiza+sYXLCzB40wfpzYf98yCyknYrsup8jr2mi5Vdf
# xW8jrfMB6Qwhf/NFk9QLCndovD/Bp56YxmZkakpv+YUmn+BS3QkXEWDOLMFdYJ5R
# 17EAGicm+eAsd59DewV7YeNXexB3ryybncymybZL8wUBUgYG9a1fUjtVE8sYWrIQ
# 9omGQxCbyt+/zQt695ov9BfzB996+UukUWrk96zbOv9HGYzZVmruySP1rriku2To
# r4iKbFLr20n6neGRoYICojCCAp4GCSqGSIb3DQEJBjGCAo8wggKLAgEBMGgwUjEL
# MAkGA1UEBhMCQkUxGTAXBgNVBAoTEEdsb2JhbFNpZ24gbnYtc2ExKDAmBgNVBAMT
...
# KoZIhvcNAQkFMQ8XDTE5MDQyMzA2MDQyNVowIwYJKoZIhvcNAQkEMRYEFLd8oN72
# zinhLwbcZeEnaE2UMIh0MIGdBgsqhkiG9w0BCRACDDGBjTCBijCBhzCBhAQUY7gv
...
# AQBD5WGm4LIlgBR5T1PhCPLcw2cYfSX8X0yFlX/w3Prq+I+qtV5gSeg8A+xW12Gs
# zd7cJT64je0T0F7PCO4SshPchNPbuZfTZnkiH50x4Q64qQpQKNfVgQTX1mNH6BDB
# dl6Sy1AsbMNsUFV0unZU780glzAXv8+UhByKs9e0nMfn7yCWuEe/rnQ53e/uY30e
# CLVDnHTyFaDkmQcC9Fv+ezHkEupx/8tuJIEYcnHi1WLjczkodpbLdDSpdKBvbzw+
# IpoHFyJOYtuO3x+08QNXejs63QWclWf56KA59LFv4CROutIjvcF/XfQnu1udBVtu
# HLrHi6xesgVPr31T+F6/m6hZ
