########################################################
#Configuring Virtual Install Disk
###################################################----#

<#
.Synopsis
    This script allows user to change the virtual install disk (UEFI / Legacy) of HPE Proliant Gen10, Gen9, and Gen8 servers

.DESCRIPTION
    This script allows user to change virtual install disk.
    VirtualInstallDisk :- Use this option to set the virtual install disk on HPE ProLiant Gen10\Gen9\Gen8 servers.
...
.EXAMPLE
    ...
    -VirtualInstallDisk   :- Accepted values are Enabled or Disabled. For multiple servers values should be separated by comma(,)

.EXAMPLE
    ConfigureVirtualInstallDisk.ps1 -Address "10.20.30.40" -Credential $userCredential -VirtualInstallDisk "Enabled"

    This mode of script have input parameter for Address Credential and VirtualInstallDisk.
    ...
    -VirtualInstallDisk :- Use this Parameter to specify virtual install disk. Accepted values are Enabled or Disabled
...
Param(
    ...
    # VirtualInstallDisk (Enabled or Disabled)
    [String[]]$VirtualInstallDisk  
     )
...
Write-Host "This script allows user to change virtual install disk."
Write-Host ""
...
foreach($IPAddress in $ListOfAddress)
{
    ...
    if($connection -ne $null)
     {  
        ...
        {
            Write-Host "Virtual install disk is not supported on Server $($connection.IP)"
...
# Get current Virtual install disk

Write-Host ""
Write-Host "Current Virtual Install Disk configuration" -ForegroundColor Green
Write-Host ""
$counter = 1
foreach($serverConnection in $ListOfConnection)
{
    $result = Get-HPEBIOSAdvancedSystemROMOption -Connection $serverConnection
...
$parameterMetaData = $(Get-Command -Name Set-HPEBIOSAdvancedSystemROMOption).Parameters
$virtualInstallDiskValidValues = $($parameterMetaData["VirtualInstallDisk"].Attributes | where {$_.TypeId.Name -eq "ValidateSetAttribute"}).ValidValues
...
Write-HOst ""
if($VirtualInstallDisk.Count -eq 0)
{
    $tempVirtualInstallDisk = Read-Host "Enter Virtual Install Disk [Accepted values : ($($virtualInstallDiskValidValues -join ","))]."
    $VirtualInstallDisk = $tempVirtualInstallDisk.Trim().Split(',')
    if($VirtualInstallDisk.Count -eq 0)
    {
        Write-Host "VirtualInstallDisk is not provided`n Exit......."
        exit
    }
}
...
{
	if($($virtualInstallDiskValidValues | where {$_ -eq $VirtualInstallDisk[$i]}) -eq $null)
	{
		Write-Host "Invalid Virtual install disk" -ForegroundColor Red
		Write-Host "Exit...."
		exit
	}
}

Write-Host "Changing Virtual install disk....." -ForegroundColor Green

$failureCount = 0
if($ListOfConnection.Count -ne 0)
{
	$setResult =  Set-HPEBIOSAdvancedSystemROMOption -Connection $ListOfConnection -VirtualInstallDisk $VirtualInstallDisk
    foreach($result in $setResult)
    {
	    if($result.Status -eq "Error")
	    {
		    Write-Host ""
		    Write-Host "Virtual install disk Cannot be changed"
...
{
	Write-Host ""
	Write-host "Virtual Install Disk changed successfully" -ForegroundColor Green
	Write-Host ""
	$counter = 1
	foreach($serverConnection in $ListOfConnection)
	{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAEclwZoTuyDLHF
# 5YZ77w4PakLmC0q4qviYenPR7u3UKKCCHuQwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# AQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCAFydOUktWv
# jZiP1yAoQT6w4W2zDS2s0ZaPxBSS3VOsgzANBgkqhkiG9w0BAQEFAASCAQCzuRe7
# +mprVeiV4MNuXANzci/NUnd2WEDrV8mNj5Jk2quVMA4uhADWsunFAj8oDqbPFgqb
# T96XrkkcIRT6tuI3hYKmxC8NYbqWu9UbLYMq0MTgeHa9WK1lQALPA91Wgv6zUdA6
# pSCqAW6VvrRsUllmwwycjF+xPWWDJJJnR1+q279MlSRaAj8yyO6l4xG8T6Cl454g
# R4RLCI7gOw+z1Mb//7e8M9/k+TEUMhoGN0tIXuNW79x3JTYE8nHKDNu4JzkgDITV
# E2SJ3pj200pP6iQA7MKxWVviWHgjpaowRsEbOHvoCEeUgMfjvacfJEqRlBFRhYg3
# IAydugH6T/2qi5eooYICojCCAp4GCSqGSIb3DQEJBjGCAo8wggKLAgEBMGgwUjEL
# MAkGA1UEBhMCQkUxGTAXBgNVBAoTEEdsb2JhbFNpZ24gbnYtc2ExKDAmBgNVBAMT
...
# KoZIhvcNAQkFMQ8XDTE5MDQyMzA2MTkwNFowIwYJKoZIhvcNAQkEMRYEFG73wNUb
# uFO0dvA8NEpy/gf+5OGJMIGdBgsqhkiG9w0BCRACDDGBjTCBijCBhzCBhAQUY7gv
...
# AQAqnDqj/LemZLMPTPReIeipdPrrJt7Z/hEdsGLz1sNaTzL4Mv7bPORiGPKT9suT
# AaqEVKBFWosr/VhFC74zLCuf4Z0oPUIZ/s3QUvDvhRMxwMO08wEQqC5Z3x/lAPJn
# nyG4qMUODTY5Kpvgbee6eLYxQZXIMr16+gYTtr9sTynhH4xHktaj6GJgJx4TLCve
# 45sxDvWi9ht3ZlYRYP7NkLwQrAWCpdZ+BCcdyd92XPaCZy6T7GmmW6jub8MeoZfE
# 3m5Uw4VwAQUos+IKGCD8iv4QsLbNYImYyHI7VmIzq/v8oMS837H1+BhPyjpEpQ8h
# VKTJDCUjPR+m1dbyAw7VKFRH
