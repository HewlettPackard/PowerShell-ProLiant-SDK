#################################################################
#Configuring Core Boosting on Gen10 servers with Intel processors
############################################################----#

<#
.Synopsis
    This script allows user to change the core boosting on HPE Proliant Gen10 servers with Intel processors.

.DESCRIPTION
    This script allows user to change the core boosting.
    CoreBoosting :- Use this option to set the core boosting on HPE Proliant Gen10 servers with Intel processors.
...
.EXAMPLE
    ...
    -CoreBoosting   :- Accepted values are Enabled or Disabled. For multiple servers values should be separated by comma(,)

.EXAMPLE
    ConfigureIntelCoreBoosting.ps1 -Address "10.20.30.40" -Credential $userCredential -CoreBoosting "Enabled"

    This mode of script have input parameter for Address Credential and CoreBoosting.
    ...
    -CoreBoosting :- Use this Parameter to specify Core Boosting technology settings. Accepted values are Enabled or Disabled
...
Param(
    ...
    # CoreBoosting (Enabled or Disabled)
    [String[]]$CoreBoosting  
     )
...
Write-Host "This script allows user to change Core Boosting."
Write-Host ""
...
foreach($IPAddress in $ListOfAddress)
{
    ...
    if($connection -ne $null)
     {  
        ...
        {
            Write-Host "Core Boosting is not supported on Server $($connection.IP)"
...
# Get current core boosting

Write-Host ""
Write-Host "Current Core Boosting configuration" -ForegroundColor Green
Write-Host ""
$counter = 1
foreach($serverConnection in $ListOfConnection)
{
    $result = Get-HPEBIOSAdvancedPerformanceTuningOption -Connection $serverConnection
...
$parameterMetaData = $(Get-Command -Name Set-HPEBIOSAdvancedPerformanceTuningOption).Parameters
$coreBoostingValidValues = $($parameterMetaData["CoreBoosting"].Attributes | where {$_.TypeId.Name -eq "ValidateSetAttribute"}).ValidValues
...
Write-HOst ""
if($CoreBoosting.Count -eq 0)
{
    $tempCoreBoosting = Read-Host "Enter Core Boosting [Accepted values : ($($coreBoostingValidValues -join ","))]."
    $CoreBoosting = $tempCoreBoosting.Trim().Split(',')
    if($CoreBoosting.Count -eq 0)
    {
        Write-Host "CoreBoosting is not provided`n Exit......."
        exit
    }
}
...
{
	if($($coreBoostingValidValues | where {$_ -eq $CoreBoosting[$i]}) -eq $null)
	{
		Write-Host "Invalid core boosting" -ForegroundColor Red
		Write-Host "Exit...."
		exit
	}
}

Write-Host "Changing core boosting....." -ForegroundColor Green

$failureCount = 0
if($ListOfConnection.Count -ne 0)
{
	$setResult =  Set-HPEBIOSAdvancedPerformanceTuningOption -Connection $ListOfConnection -CoreBoosting $CoreBoosting
    foreach($result in $setResult)
    {
	    if($result.Status -eq "Error")
	    {
		    Write-Host ""
		    Write-Host "Core boosting Cannot be changed"
...
{
	Write-Host ""
	Write-host "Core Boosting changed successfully" -ForegroundColor Green
	Write-Host ""
	$counter = 1
	foreach($serverConnection in $ListOfConnection)
	{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA/jEmoqunFC+7L
# Vz6DOTzDOB40Dh4GdobUeR6eEcHOIKCCHrMwggPuMIIDV6ADAgECAhB+k+v7fMZO
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgzbXMlgCY
# CMF/WXcb/AmzuBT4QbVFiAIssUS+XR3elU4wDQYJKoZIhvcNAQEBBQAEggEAUK8z
# aS8UhUQDEtkRSXVveNyHjrHNQNRY6c/dOsJ9Piyyq1H+5KtglhvlcGpqrw6yal8p
# lFlgWyalcunetDQ/BG09eDRiiooyGeYfEV+YOxgaE+r0chsb25Iy//OTyezNP3VT
# atMf+YflPwOKTukDsgWIgdrZt2/AprInyD8tVp3grkg9V1CQhYbjdFHYWbmd3TES
# X1ByBt8VopAl572VR7h0L2uOaaMQk6ejrFpOQ1WIoryNirXylMusEUvy9Zwwzc2U
# Yq3zSBs1umQ6AvH8iDzGW+gc9NHxZZHVINaJdXC5ZL55JujqZuY2ipZH0PeEFSS8
# XxJIxI+v5SJTHJH27qGCAgswggIHBgkqhkiG9w0BCQYxggH4MIIB9AIBATByMF4x
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0xODAzMDkxMDA5NThaMCMGCSqGSIb3DQEJ
# BDEWBBRB+d2rQWCh4xtnwlSoJHWKwq09czANBgkqhkiG9w0BAQEFAASCAQA7Jz8H
# ydNmj2TAwdwt7YFW1jKru7FZDYuuwbwVUn9gfDLs1hEEOcnv2DzgQpBiAAgntQ/h
# EQDF1i/FDui0rVSL4edum43xuIIi51kvAJ4A7/pIimp8ccsb56mJMDnU6vQY1mTL
# wCyEwAMJ5FKMlngIgkuEonn4mscH9el7IxROWA1HBLhQD9uVz343yoG6PaoGUK8m
# X8bHYiKSWoKUkGMKOKeAOy5LvTFQSQKATrvp7lg6N2sR+PWLwgcsct8R9SuqY9Xe
# cjXAVcFzJOa1nS6VhurnsAMuxQ09pcXwBeL5nzrdGuwMN3Ha8g97QO2/5QJA06NO
# KCes56Co7fL9IkeT
