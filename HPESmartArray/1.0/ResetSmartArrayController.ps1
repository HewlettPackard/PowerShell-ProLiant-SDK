########################################################
#Reset the Smart Array controller.
###################################################----#

<#
.Synopsis
    This script allows user to reset the smart array controller on the target server.

.DESCRIPTION
    This script allows user to reset smart array controller. Specify the controller location where the smart array controller is installed.
    

.EXAMPLE
    ResetSmartArrayController.ps1
    ...
    ControllerLocation :- Specify the controller location where the smart array controller is installed.

.EXAMPLE
    ResetSmartArrayController.ps1 -Address "10.20.30.40" -Credential $userCredential
...
Write-Host "`nThis script resets the smart array controller."
...
#Check SmartArray cmdlets module is installed or not.
...
else
{
    ...
    Write-Host "$HPESmartArrayCmdlets Module Version : $($InstalledSAModule.Version) is installed on your machine.`n"
}
...
{
    $Credential = Get-Credential -Message "Enter Username and Password (Use same credential for multiple servers)`n"
}
...
foreach($serverConnection in $ListOfConnection)
{
    ...
    $controllerLocation = Read-Host "Enter ControllerLocation."
    $controllerLocationList = $find.SmartArrayController.ControllerLocation
    if($controllerLocationList -contains $controllerLocation){
        Write-Host "`nSmart Array controller being reset..." -ForegroundColor Green
        $Resetresult = $c | Reset-HPESAController -ControllerLocation $controllerLocation -FactoryReset
        $Resetresult
    }
    else{
        Write-Host "Invalid ControllerLocation." -ForegroundColor Red
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC1jfdMTCRXRaev
# m7RB1O2BX5Wa8xXmxplq30rZi3i1lKCCHrMwggPuMIIDV6ADAgECAhB+k+v7fMZO
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgQjgoTVcL
# RKnddzHS0cWZwPMzDu3LJnCpcftuF39FbRIwDQYJKoZIhvcNAQEBBQAEggEAk+bD
# gY1cvn3pwYeEV+EXZMUlZBUWaKnFPgpd1oXCDn3Od4glS9Hj3CvFMakPLZ57pA6A
# Ga0ZwRgqOye0imIh86tMX3zd8mGJOb+G0wSCPL0Y4QIhUmM+qFgd05F1LCQDZnBm
# B3pOYyfKgvitWPjmcchRTw52sBtM50QbAB46O5PEnZlknTcqmlhgWPD5KsYfvp5D
# Dus1tQLBvn8hdbOkjBc2M19rtQLzZTFQHgQj7q/TN/3sZewdBbL6b1P7ltdJgBhp
# qC9m4Qo/DolWCxZezsJ8JK+o+vPtDNaoSaYyRnep+tZMzbvbrjX7SayQh0/p1flA
# pEOnjy9dMTnqdL7ipqGCAgswggIHBgkqhkiG9w0BCQYxggH4MIIB9AIBATByMF4x
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0xODAzMDgwOTUzMDdaMCMGCSqGSIb3DQEJ
# BDEWBBQxZLDjtEM9v0EZ/qoPWB2A4AMVfzANBgkqhkiG9w0BAQEFAASCAQBoFuC0
# RtRko/CszaneuNl0vIwuz4neSRW+jDOvqbY/yPfX3TMOnserCIbwFU9Dy7wxqpSL
# uA+UljxOlJQLkq8kA20+mjEcOFF6azpcVfN/KAB1Q7LgQBdwk5h0rDPpGY7XNW66
# odPfjAk5DxgTS17PWS7YGFozs5q+YnOHBtB9gkVK6lj2FqJcfb0xa+zaYSNCPc5v
# ROua0GdNWqywZYSm7mH700l8BN9R1SL98fQ9KckjjvcDRdL0EfC4nb6k1SAnLMMj
# oIAJYOMaF9RZkGqrrQ+5kpYy27X6c8FyA9+lrENhs3cz2SgHRd4tUf9zgv/7Qhr9
# mPzT8b914nEQOKtC
