########################################################
#Gets physical and logical drives details
###################################################----#

<#
.Synopsis
    This script allows user to get physical and logical drives details.

.DESCRIPTION
    This script allows user to get physical and logical drives details.
    Physical drive information like Size, interface  type, media type firmware version, health status etc.
    Logical drive information like Size, Physical drive, Spare drive, Raid level etc.
...
Write-Host "`nThis script gets physical and logical drives details."
...
#Load HPESmartArrayCmdlets module
...
{
    $tempAddress = Read-Host "`nEnter server address (IPv4/IPv6 or Hostname). Multiple entries separated by comma (,)"
    $Address = ($tempAddress.Trim().Split(','))
}
    
if($Address.Count -eq 0){
    Write-Host "`nYou have not entered IP(s) or Hostname(s)`nExit"
    exit
}
...
#Get Physical and logical drive details
foreach($serverConnection in $ListOfConnection)
{
    $physicalDrive = Get-HPESAPhysicalDrive -Connection $serverConnection
    $logicalDrive = Get-HPESALogicalDrive -Connection $serverConnection
    Write-Host "------------------------ Server $($serverConnection.IP) ------------------------`n" -ForegroundColor Yellow
    Write-Host "`n*** Physical drives ***`n" -ForegroundColor Magenta
    if($physicalDrive.PhysicalDrive.Count -eq 0){
        Write-Host "`tNo physical drive found." -ForegroundColor Red
    }
    else{
        $physicalDrive.PhysicalDrive
    }

    Write-Host "`n*** Logical drives ***`n" -ForegroundColor Magenta
    if($logicalDrive.LogicalDrive.Count -eq 0){
        Write-Host "`tNo logical drive found." -ForegroundColor Red
    }
    else{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCdjRCm8C9NGvOq
# G0IffV/AMD5WvMsTnHqj083EIPqFBqCCHrMwggPuMIIDV6ADAgECAhB+k+v7fMZO
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgZpcWeXej
# TprCNs+bT740gse5dTwZnigwnI9MWwlY4n0wDQYJKoZIhvcNAQEBBQAEggEAeMO2
# GNHvgzMdnlKo8BJ54OZi6AD0s7h6YQgJcJ7i1E8kz9EZz2gFskkXr+6z5EsBSYPS
# TZkesbXnfrVl1DhiKSjyIideOisAVfDBrVNZwXoVGIIFZDFe+XBDsyVgfhlndtg8
# mWst8uaDFycFlPmbz8+jMmsT753aqjr0VPwYYqQIjpAocb3xWwHYuCK2ZE9eiR06
# kibc7kTNEUrrpT6VDFAGS7n7Zfc3U1AFYkwxaethow5FLR3jOkaweZF0rut2jykO
# H/7Fxqhlnchyxgk08d1/Z2SMxbUVyPB32JmGdy1BTl/hPnWlRLyL1CHd0WhuD5uQ
# IXuLRNQjxOT4wG43L6GCAgswggIHBgkqhkiG9w0BCQYxggH4MIIB9AIBATByMF4x
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0xODAzMDgwOTUyMTZaMCMGCSqGSIb3DQEJ
# BDEWBBRgJeIrP8E8+/z+q5avNe2oKMYOvjANBgkqhkiG9w0BAQEFAASCAQCGFeL7
# UM7zbCNE42Dd0OTzemPnoKTR8UcnA4uA8OFEQUfUHjFJiJbllehLASxppoyV/HYO
# DDwFcuSWhRNw8sWooXTKMEtux0V4W5ZiGpzIRnwIQULUyOaTlaSatDNsNPm7Bij5
# f09C2XsSdCShreVmZjNmn7pfK7AqzqdtuDYja/hTnmJpQDWR08iU/Q90iUg+f1Nv
# Pmi3+g8sRmYx14l4gsno+4/np/eoue7bPBvSKDZFnlo+w6xpF1DJOw4M7lnnPUUg
# TZ02Yo9j9LVs0eqRPqpDb/txQLZMAe6MCUh1DrBpBooq1XdF7w+gxLLdB5jzPzoE
# BUV9jO8YJgFeMKsD
