########################################################
#Creates new logical drive
###################################################----#

<#
.Synopsis
    This script allows user to create new logical drive on the specified controller.
.DESCRIPTION
    This script allows user to create new logical drive on the specified controller.
    This script gives you the detailed information which are required to create logical drive like controller location, physical drive location.
.EXAMPLE
    CreateLogicalDrive.ps1
    ...
    LogicalDriveName :- Specify the logical drive name.

    Raid :- Specify the Raid level.

    DataDrive :- Specify the data drive (Physical drive).

.EXAMPLE
    CreateLogicalDrive.ps1 -Address "10.20.30.40" -Credential $userCredential
...
Write-Host "`nThis script creates new logical drive."
...
if($Credential -eq $null){
    $Credential = Get-Credential -Message "`nEnter Username and Password (Use same credential for multiple servers)"
...
foreach($serverConnection in $ListOfConnection)
{
    ...
    $listOfSlot = $find.SmartArrayController.ControllerLocation
    $controllerLocation = Read-Host "`n Enter ControllerLocation [Accpted Values are ("($listOfSlot -join ', ' )")]"
    if($listOfSlot -contains $controllerLocation)
    {
       $logicalDriveName = Read-Host "`n Enter LogicalDriveName"
       $raidLevel = Read-Host "Enter Raid Level [Accepted values (Raid0, Raid1, Raid1ADM, Raid10, Raid10ADM, Raid5, Raid50 ,Raid6 ,Raid60 )]"
       $physicalDrive = Get-HPESAPhysicalDrive -Connection $serverConnection -ControllerLocation $controllerLocation
       $PhysicalDriveList = $physicalDrive.PhysicalDrive.Location
       Write-Host "Input Hint : eneter multiple physical drive separated by comma(,)" -ForegroundColor Yellow
       $userInputdataDrive = Read-Host "`n Enter DataDrive [Accepted Values("($PhysicalDriveList -join ', ')")]"
       $datadrive = $userInputdataDrive.Split(',')

       foreach($item in $datadrive){
        if($PhysicalDriveList -notcontains $item){
           Write-Host "`n Invalid physical drive : $item `n Logical drive cannot be created." 
        }
       }

       $newLogicalDrive = New-HPESALogicalDrive -Connection $serverConnection -ControllerLocation $controllerLocation -LogicalDriveName $logicalDriveName -Raid $raidLevel -DataDrive @(,$datadrive)
       $newLogicalDrive.StatusInfo.Message
    }
    else 
    {
        Write-Host "`nInvalid ControllerLocation : $controllerLocation" -ForegroundColor Red
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA/4OmcYdfS9A1z
# n8shf/FO5fg9CnHMYdiME3w79KT0/KCCHrMwggPuMIIDV6ADAgECAhB+k+v7fMZO
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgrcPXfyDF
# Ng9+GXguudUujpGyAZcLz8p17c4qmvsxtXIwDQYJKoZIhvcNAQEBBQAEggEAbhhd
# BKjCyriSwV/ivCiMpinH6VEHBBwYvvFRntBCfiwkpx/UlmYo/bu3ZFPNfvGM/htk
# 4GnNemQRgRm6oebJmrO8cDbPjiIQOZFJVXStcWHrsWHsxoj+Qs3J8zrCs/SsIxv5
# BfI5hXXkYzpM3uqn5z+twamqRloV2QpcEn3XYKYeeqqOkH7FSKIlLBMHLk6QwiSY
# w5Ys8huLx+4TcUsGP/sOZ9Vml1UlYqPBlF6bWI8bxCvZBWDjdzE/GrM1pG/m0xPf
# 552xXwF0CwGExvcZ/4LP6amSGNcaT/4juqeWfOd5DXwlmoK/k1k+sJ6F5GyeP8ML
# 4olXCqfZ9DIqfQdKrqGCAgswggIHBgkqhkiG9w0BCQYxggH4MIIB9AIBATByMF4x
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0xODAzMDgwOTUxNTBaMCMGCSqGSIb3DQEJ
# BDEWBBRQ6ANXAVpqLbLxMhgUtL4Zk6tTKTANBgkqhkiG9w0BAQEFAASCAQARwGbK
# vsXnLwgV8D2bRNkxora2GeTiqoAEHpl0pFmt7Fwmw+OnEd5NPG5gereSlgSROEuo
# TYH1AkjUfGT1ZXWTRyzLZohzK0oAqBM4jljWG2GA0fxufYQ8Y84df9QFAzu+chxw
# eW5VQ9LvgmTlBrpjX+tZaOq1xuA4AvV9DqbLPE+69dyEAyJG0y/g3clp7XhVcCmp
# vyr2hhgs39l8UbJBYdP/nEt7Sqc2JKbrQI97isGLvCVy54pidbmd70ENVwY1RM4v
# P0WgkCq/xDV/WxV3U7xi2zmRCuL7bfDtbxf4za9fPxABtmKsDKQZqEtjHpmNU66g
# PBzksePUnVn22FD3
