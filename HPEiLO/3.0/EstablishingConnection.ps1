...
try
{
	...
	{
        foreach($object in $Connection)
        {
            $object | fl 

            Write-Host "`nTarget info for $($object.IP):" -ForegroundColor Green
            $object.TargetInfo | fl

            Write-Host "`nExtended info for $($object.IP):" -ForegroundColor Green
            $object.ExtendedInfo | fl
        }
	}
    ...
    foreach($data in $testConnection)
    {
        $data | fl
        Write-Host "`nTest connection details for $($data.IP):" -ForegroundColor Green
        $data.TestConnectionDetail |fl
    }
}
catch
{
